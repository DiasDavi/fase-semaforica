...
async function loadDetectorConfig() {
    try {
        const response = await fetch('/detect/config', { method: 'GET' });
...
async function updateConfig(iou, confidence) {
    ...
    try {
        await fetch('/detect/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(configData) });
    } catch (error) {
